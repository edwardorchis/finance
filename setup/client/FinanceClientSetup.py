...
class WinMain(Frame):
    def __init__(self, parent=None):
        ...
        self._lab_title.set('欢迎使用 Finance Client')
    ...
    def _run_click(self):
        print('"' + self._dst_path.get() + '\\client\\FinanceClient.exe"')
        os.popen('"' + self._dst_path.get() + '\\client\\FinanceClient.exe"')
        # 运行主程序
        self.root.destroy()
    ...
    def _set_config(self):
        # 检查config文件是否存在
        if (os.path.exists(self._dst_path.get() + "\\client\\FinanceClient.exe.config")):
    ...
    def _set_uninstall(self):
        try:
            ...
            winreg.SetValueEx(reg, 'DisplayIcon', 0, winreg.REG_SZ, self._dst_path.get() + '\\client\\FinanceClient.exe')
            ...
            winreg.SetValueEx(reg, 'UninstallString', 0, winreg.REG_SZ, self._dst_path.get() + '\\client\\unist.exe')
        except Exception as e:
            print(e)
        finally:
            winreg.CloseKey(reg)
    ...
    def _create_desktop_link(self):
        ...
        try:
            target_exe = self._dst_path.get()+'\\client\\FinanceClient.exe'
    ...
    def _update_config(self, key, value):
        client_config_file = self._dst_path.get() + "\\client\\FinanceClient.exe.config"
        if (key == 'server_url'):
